...
if (length(args) != 3) stop('Usage: Rscript score.R input_directory output_directory reference_directory')
input_root <- args[1]; root <- args[2]; reference_root <- args[3]
for (folder in c('results','provenance')) dir.create(file.path(root,folder),recursive=TRUE,showWarnings=FALSE)
...
ltm <- readRDS(file.path(reference_root,'external_data','ligand_target_matrix_nsga2r_final_mouse.rds'))
lr <- readRDS(file.path(reference_root,'external_data','lr_network_mouse_21122021.rds'))
from <- if ('from' %in% names(lr)) 'from' else 'ligand'
to <- if ('to' %in% names(lr)) 'to' else 'receptor'
inp <- read.csv(file.path(input_root,'data','official_nichenet_window_inputs.csv'),check.names=FALSE)
