"""Numerical plotting functions for the MOSTA paper panels.

Adapted from the figure source in release_artifacts,
reproduction/si/S9_S10/source/render_mosta_s9_s10_clusterprofiler_exact_submitted_style.py.
Calculations, panel selection, and data colors are unchanged.
"""
...
def render_s9(go_dir: Path, output_dir: Path) -> tuple[dict[str, Path], dict[str, object]]:
    ...
    outputs = save_figure(
        fig,
        output_dir,
        "Figure_S16_MOSTA_gene_enrichment",
    )
...
def render_wave_axes(
    ...
    ax_strip.set_xticks([])
    ax_strip.set_yticks([])
    ax_strip.set_title("Phase", fontsize=7.5, pad=5, color="black")
    ...
    for spine in ax.spines.values():
        spine.set_visible(False)
    fig.text(0.126, 0.817, "Top 1000 dynamic genes", rotation=90, ha="center", va="center", fontsize=8.5, color="black")
    vector_colorbar(
        fig,
...
def render_s10(shared: Path, go_dir: Path, output_dir: Path) -> tuple[dict[str, Path], dict[str, object]]:
    ...
    outputs = save_figure(
        fig,
        output_dir,
        "Figure_S17_MOSTA_developmental_waves",
    )
