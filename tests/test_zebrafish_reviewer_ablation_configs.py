...
def _plan_contract(config: dict) -> list[dict]:
    ...
    return [
        {key: value for key, value in stage.items() if key not in ignored}
...
def test_no_lr_prior_changes_only_edge_gate_and_output_directory() -> None:
    ...
    no_lr = _load("zebrafish_spatial_full_alpha_express_0015_no_lr_prior.yaml")
...
def test_no_interaction_preserves_retained_nets_and_training_budget() -> None:
    ...
    assert [stage["epochs"] for stage in no_interaction["training"]["plan"]] == [
        100,
        100,
        50,
        2001,
    ...
    assert all(
        "i" not in stage["train_strategy"] and stage["interaction_use"] is False
