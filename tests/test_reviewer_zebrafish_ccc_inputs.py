...
def _fixture(tmp_path: Path) -> tuple[Path, Path, Path, np.ndarray]:
    ...
    data.obsm["spatial_aligned"] = np.asarray([[0, 0], [1, 0], [0, 1], [1, 1]], dtype=float)
...
def test_primary_preparation_uses_audit_target_and_matches_frozen_x(tmp_path: Path) -> None:
    ...
    assert prepared.adata.uns["ccc_preprocessing"]["interaction_graph"]["neighborhood_threshold"] == 0.4
...
def test_bundle_writes_same_cells_expression_and_spatial_for_all_methods(tmp_path: Path) -> None:
    ...
    assert manifest["preprocessing"]["interaction_graph"]["recommended_spot_diameter"] == 0.1
