...
def plot_3d_spatial_sankey_style_focus_anchor(
    adata_dict,
    ...
    def _resolve_annotation_col(ad) -> str:
        cols = list(ad.obs.columns)
        low_to_col = {c.lower(): c for c in cols}

        candidates: List[str] = []
        if annotation_key is not None:
            k = str(annotation_key).strip()
            if k:
                candidates.extend([k, k.lower()])
        candidates.extend(["Annotation", "annotation", "cell_type", "celltype_prediction", "region"])
        ...
        for c in candidates:
            ...
            lc = c.lower()
            if lc in low_to_col:
                return low_to_col[lc]
        raise KeyError(
            "Could not resolve annotation column for 3D plotting. "
            f"Tried={candidates}; available={cols}"
        )
    ...
    def create_arrowhead_segments(
        ...
        if reference_length is None:
            reference_length = direction_len
        edge_len = float(reference_length)
        if edge_len < 1e-6:
            return None
        ...
        if plane_normal is not None:
            ...
            if plane_norm >= 1e-6:
                ...
                perp = np.cross(plane_normal, direction)
        ...
        if abs(float(np.dot(direction, ref1))) > 0.9:
            ref1 = np.array([0.0, 1.0, 0.0])
        perp1 = np.cross(direction, ref1)
        ...
        base = p_tip - direction * arrow_len
        wing_points = [
            base + perp1 * arrow_width,
            base - perp1 * arrow_width,
            base + perp2 * arrow_width,
            base - perp2 * arrow_width,
        ]
            ...
            for e in edges_list:
                ...
                if edge_render_mode == "line":
                    ...
                    if curve_points is not None:
                        ...
                        curve_segments = np.diff(np.asarray(curve_points, dtype=float), axis=0)
                        arrow_reference_length = float(
                            np.linalg.norm(curve_segments, axis=1).sum()
                        )
                        ...
                        arrow_frac = min(0.95, max(0.15, float(edge_arrow_position)))
                        prev_frac = max(0.0, arrow_frac - 0.12)
                        arrow_prev = [
                            p1[0] + (p2[0] - p1[0]) * prev_frac,
                            p1[1] + (p2[1] - p1[1]) * prev_frac,
                            p1[2] + (p2[2] - p1[2]) * prev_frac,
                        ]
                        arrow_tip = [
                            p1[0] + (p2[0] - p1[0]) * arrow_frac,
                            p1[1] + (p2[1] - p1[1]) * arrow_frac,
                            p1[2] + (p2[2] - p1[2]) * arrow_frac,
                        ]
                    fig.add_trace(
                        go.Scatter3d(
                            x=xs,
