...
def set_seed(seed=42):
    """Seed the process-local random-number generators used by CytoBridge.

    This function deliberately does not assign ``PYTHONHASHSEED``. CPython
    reads that variable when the interpreter starts, so changing it here
    cannot reseed hash randomization in the current process. Launch Python
    with ``PYTHONHASHSEED`` already set when stable hash iteration is required.
    """
    # Python
    random.seed(seed)
    # NumPy
...
def load_model_from_adata(adata: AnnData) -> torch.nn.Module:
    ...
    if "all_model" not in adata.uns or "model_config" not in adata.uns["all_model"]:
        raise ValueError(
            "CytoBridge model information not found. Please fit the model first."
        )
    ...
    if isinstance(training_config.get("plan"), str):
        training_config["plan"] = json.loads(
            training_config["plan"]
        )  # Convert JSON string back to list

    model_config = adata.uns["all_model"]["model_config"]
    dim = int(
        adata.uns["all_model"].get("model_input_dim", adata.obsm["X_latent"].shape[1])
    )
...
def save_model_to_file(model: DynamicalModel, save_path: str) -> None:
    ...
    torch.save(
        {"model_config": model.config, "state_dict": model.state_dict()}, save_path
    )
