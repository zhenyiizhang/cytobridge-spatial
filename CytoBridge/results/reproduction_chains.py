...
FIGURE_REPRODUCTION_CHAINS: dict[str, tuple[dict[str, str], ...]] = {
    ...
    "nonspatial": (
        ...
        _row(
            "S4 Weinreb; S5 scNT",
            "fit Full and No-interaction arms",
            "cytobridge nonspatial train --dataset <weinreb|scnt_cortex> --arm full --preprocess-manifest <run>/preprocess/preprocess_manifest.json --edge-prior-manifest <run>/edge_prior/manifest.json --output-dir <run>/full --device cuda:0; repeat with --arm no_interaction, omit --edge-prior-manifest, and write <run>/no_interaction",
            "preprocess_manifest.json; full-arm edge-prior manifest; arm-specific configuration",
            "<run>/<arm>/model checkpoints; resolved configuration; training summary",
            "matched evaluation",
        ),
        _row(
            "S4c/S5c",
