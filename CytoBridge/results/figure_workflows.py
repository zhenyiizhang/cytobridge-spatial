...
FIGURE_WORKFLOWS = (
    ...
    FigureWorkflow(
        ...
        "python scripts/collect_training_history_inputs.py --run zebrafish=<zebrafish-run>/training --run mosta=<mosta-run>/training --run arista=<arista-run>/training --run admouse=<admouse-run>/training --run chicken_heart=<heart-run>/training --output-dir <s41-inputs>",
        "cytobridge figure training-histories --output-dir outputs/training_histories",
        "Redraws S41 from the histories written by completed model runs.",
    ),
    FigureWorkflow(
        ...
        "Redraws S42 after ROI selection and permutation-table calculation.",
    ),
    FigureWorkflow(
        "zebrafish-attention",
