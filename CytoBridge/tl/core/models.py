...
from CytoBridge.tl.core.interaction import ExpNormalSmearing, CosineCutoff, cal_interaction
ACTIVATION_FN = {
    'relu': nn.ReLU,
...
class InteractionModel(nn.Module):
    def __init__(self, x_dim,n_layers=2 , hidden_dim=400, activation='leaky_relu', num_rbf=16, cutoff=1, dim_reduce = False, residual = False):
        ...
        self.cutoff = cutoff
        self.rbf_expansion = ExpNormalSmearing(cutoff=cutoff, num_rbf = self.num_rbf, trainable= False)
...
class DynamicalModel(nn.Module):
    ...
    def _expand_time_like(self, t: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        ...
        if t.shape[0] == 1 and x.shape[0] > 1:
            ...
            raise ValueError(f"Time rows ({t.shape[0]}) must match x rows ({x.shape[0]})")
        return t
