...
_SCORE_ENERGY_OBJECTIVES = frozenset(
    {
        "legacy_by_interaction",
        "velocity_score_cross_term",
        "growth_score_correction",
    }
)


def _validate_score_energy_objective(value):
    """Return a supported score-energy objective or fail closed.

    ``legacy_by_interaction`` preserves the historical behavior in which the
    score-energy integrand changed when the interaction component was active.
    The two fixed modes make that choice independent of the interaction gate,
    which is required for an ablation that removes only interaction.
    """
    if not isinstance(value, str) or value not in _SCORE_ENERGY_OBJECTIVES:
        choices = ", ".join(sorted(_SCORE_ENERGY_OBJECTIVES))
        raise ValueError(
            f"score_energy_objective must be one of: {choices}; got {value!r}."
        )
    return value
...
def neural_ode_step(ODE_func, x0, lnw0, t0, t1, device):
    ...
    x_t, lnw_t, e_t = odeint(
        ODE_func, initial_state, time, method="euler", options=dict(step_size=0.1)
    )
...
class ODEFunc(nn.Module):
    def __init__(
        ...
        self.set_score_energy_objective(score_energy_objective)

    def set_score_energy_objective(self, value):
        """Select the score-energy integrand independently of model gating."""
        self.score_energy_objective = _validate_score_energy_objective(value)

    def forward(self, t, state):
        x, lnw, m = state
        batch_size = x.shape[0]

        # Do not evaluate an inactive interaction component: besides wasted
        # compute, stochastic grouping would otherwise advance an RNG stream
        # for a term whose output is discarded.
        outputs = self.model(
            t,
            x,
            lnw,
            except_interaction=self.interaction_use,
            interaction_generator=(
                self.interaction_generator if self.interaction_use else None
            ),
        )
        v = outputs["velocity"]
        ...
        has_score = self.score_use and "score" in outputs
        if self.score_energy_objective == "legacy_by_interaction":
            use_velocity_score_cross_term = (
                has_score
                and self.interaction_use
                and "interaction" in outputs
                and self.model.interaction_net.cutoff != 0
            )
        else:
            use_velocity_score_cross_term = (
                self.score_energy_objective == "velocity_score_cross_term"
            )

        if has_score and not use_velocity_score_cross_term:
            s = outputs["score"]
        ...
        # print("V+G+s/V+s")
            ...
            grad_s = outputs["score_gradient"]
            norm_grad_s = (
                torch.norm(grad_s, p=2, dim=1).unsqueeze(1).requires_grad_(True)
            )

            de_dt = (
                torch.norm(v, p=2, dim=1).unsqueeze(1) ** 2 / (2)
                + (norm_grad_s**2) / 2
                + torch.norm(v, p=2, dim=1).unsqueeze(1)
                * torch.norm(grad_s, p=2, dim=1).unsqueeze(1)
                + g**2
            ) * torch.exp(lnw)
            # print("V+S+G+I/V+S+I")
        else:
...
class ODEFunc2InteractionEnergy(nn.Module):
    ...
    def forward(self, t, state):
        ...
        g = (
            outputs["growth"]
            if (self.use_mass and "growth" in outputs)
            else torch.zeros(z.shape[0], 1, device=z.device)
        )
...
class ODEFunc3(nn.Module):
    ...
    def forward(self, t, state):
        ...
        grad_s = torch.autograd.grad(
            outputs=s, inputs=z, grad_outputs=torch.ones_like(s), create_graph=True
        )[0]
