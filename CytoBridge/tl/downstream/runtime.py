...
class _VelocityGrowthAdapter:
    def __init__(self, model):
        self.model = model
        components = {
            str(value).strip().lower() for value in getattr(model, "components", [])
        }
...
def build_dynamical_runtime(model_or_loaded) -> DynamicalRuntime:
    ...
    components = {str(value).strip().lower() for value in model.components}
    if "interaction" in components and getattr(model, "interaction_net", None) is None:
        raise TypeError(
            "Model declares an interaction component but is missing interaction_net."
        )
    if (
        "interaction" not in components
        and getattr(model, "interaction_net", None) is not None
    ):
        raise TypeError(
            "Model has no interaction component but still exposes interaction_net; "
            "refusing an ambiguous no-interaction runtime."
        )
