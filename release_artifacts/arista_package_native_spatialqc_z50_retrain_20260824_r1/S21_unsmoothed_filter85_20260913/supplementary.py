...
def draw_lineage(data, output, palette):
    # Match Figure 5a: count direct classifier predictions, without spatial voting.
    with np.load(data / 'fixed_particle_lineage_labels_unsmoothed.npz', allow_pickle=False) as archive:
    ...
    fig = plot_sankey(
        ...
        min_flow=10, keep_source_cumfrac=.85, normalize_mode=None,
        min_flow_before_cumulative=True,
    ...
    fig.write_html(output / 'S21a_lineage.html')
    first = output / 'S21a_lineage.pdf'
    fig.write_image(str(first))
    dirs = directories(output)
    second, _ = spatial.plot_s14b(fractions, palette, 'S21b_composition', dirs, dirs['tables'], font_family='Arial')
    document = fitz.open()
    ...
    regular = findfont(FontProperties(family='Arial'), fallback_to_default=False)
    bold = findfont(FontProperties(family='Arial', weight='bold'), fallback_to_default=False)
    page.insert_font(fontname='ArialPanel', fontfile=bold)
    page.insert_font(fontname='ArialTitle', fontfile=regular)
    page.insert_textbox(fitz.Rect(50, 4, 510, 22), 'Cell Fate Transitions',
                        fontsize=8, fontname='ArialTitle', align=1, color=(0,0,0))
    for letter, point in [('a', (0, 18)), ('b', (0, 349))]:
        page.insert_text(point, letter, fontsize=20, fontname='ArialPanel', color=(0,0,0))
